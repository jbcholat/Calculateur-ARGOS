...
const PredictiveMaintenanceROICalculator = () => {
  ...
  // Global parameters
  const [totalPumps, setTotalPumps] = useState(DEFAULT_VALUES.totalPumps);
  const [argosPricePerPump, setArgosPricePerPump] = useState(DEFAULT_VALUES.argosPricePerPump);
  const [detectionPercentage, setDetectionPercentage] = useState(DEFAULT_VALUES.detectionPercentage);
  ...
  // Reset function
  const handleReset = () => {
    setTotalPumps(DEFAULT_VALUES.totalPumps);
    setArgosPricePerPump(DEFAULT_VALUES.argosPricePerPump);
    ...
    <div className="w-full h-screen bg-gray-100 flex" style={{ aspectRatio: '16/9' }}>
      ...
      <div className="w-1/4 bg-white p-4 shadow-lg flex flex-col">
        ...
        <div className="mt-6">
          <button
            onClick={handleReset}
            className="w-full py-3 px-4 font-bold text-white rounded-lg shadow-lg transition-all duration-200 hover:shadow-xl"
            style={{ backgroundColor: '#FF5800' }}
            onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#E04E00'}
            onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#FF5800'}
          >
